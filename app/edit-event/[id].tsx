...
export default function EditEvent() {
  ...
  const loadEvent = async () => {
    try {
      setIsLoading(true);
      const event = await eventsService.getEvent(id as string);
      if (event) {
        setTitle(event.title);
        const eventDescription = event.description || '';
        console.log('Descripción cargada:', eventDescription);
        ...
        setSelectedJournalId(event.shared_journal_id);
        setMetadata(event.metadata || {});
        
        // Cargar archivos multimedia existentes
        if (event.media && event.media.length > 0) {
          const mediaFiles = event.media.map((m: any) => ({
            uri: m.file_path,
            type: m.mime_type.startsWith('image/') ? 'image' : 
                  m.mime_type.startsWith('video/') ? 'video' : 'audio',
            id: m.id,
            fileName: m.file_name
          }));
          setMediaFiles(mediaFiles);

          // Cargar el primer archivo de audio si existe
          const audioFile = mediaFiles.find(file => file.type === 'audio');
          if (audioFile) {
            try {
  ...
  const removeMedia = async (index: number) => {
    ...
    if (mediaToRemove.id) {
      Alert.alert(
        ...
        [
          ...
          },
          {
            ...
            style: 'destructive',
            onPress: () => {
              // Agregar el ID a la lista de archivos eliminados
      ...
      // Si es un archivo nuevo (sin ID), simplemente lo eliminamos del estado
      const newMediaFiles = [...mediaFiles];
      newMediaFiles.splice(index, 1);
      setMediaFiles(newMediaFiles);
    }
  ...
  const handleSave = async () => {
    try {
      ...
      formData.append('type', 'mixed'); // Forzar tipo mixto
      formData.append('category', selectedCategories[0]); // Usar la primera categoría seleccionada
      formData.append('event_date', date.toISOString());
      formData.append('timezone', Intl.DateTimeFormat().resolvedOptions().timeZone);

      // Agregar shared_journal_id si existe
      if (selectedJournalId) {
        formData.append('shared_journal_id', selectedJournalId.toString());
      }
      ...
      // Agregar nuevos archivos de medios
      const newMediaFiles = mediaFiles.filter(file => !file.id);
      if (newMediaFiles.length > 0) {
        newMediaFiles.forEach((file: any, index: number) => {
          const fileType = file.type === 'image' ? 'image/jpeg' : 
                          file.type === 'video' ? 'video/mp4' : 'audio/mpeg';
          const fileExtension = file.type === 'image' ? 'jpg' : 
                              file.type === 'video' ? 'mp4' : 'mp3';
          
          formData.append('media[]', {
            uri: file.uri,
            type: fileType,
            name: file.fileName || `file${index}.${fileExtension}`
          } as any);
        });
      }

      // Actualizar el evento
      const updatedEvent = await eventsService.updateEvent(id, formData);
      ...
      // Eliminar los archivos multimedia marcados para eliminación
      if (deletedMediaIds.length > 0) {
        try {
          await Promise.all(deletedMediaIds.map(mediaId => 
            eventsService.deleteEventMedia(id as string, mediaId)
          ));
        } catch (error) {
          console.error('Error al eliminar archivos multimedia:', error);
          // No mostramos error al usuario ya que el evento se actualizó correctamente
        }
      }
      ...
      Alert.alert('Error', error.message || 'Error al guardar el evento');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Añadir estilos para el modal de bitácora
  const journalListStyle = {
    maxHeight: 300,
  };
  ...
  const journalItemActiveStyle = {
    backgroundColor: '#f0f0f0',
  };

  const journalItemTextStyle = {
    fontSize: 16,
    color: '#202024',
  };

  const journalItemTextActiveStyle = {
    fontWeight: '600',
  };
              ...
              {EVENT_CATEGORIES.map((cat) => (
                ...
                >
                  <Ionicons 
                    ...
                    size={16} 
                    color={selectedCategories.includes(cat.id) ? '#FFFFFF' : '#666'} 
                  />
                  <Text 
                    style={[
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={journalListStyle}>
              {journals.map((journal) => (
                <TouchableOpacity
                  key={journal.id}
                  style={[
                    journalItemStyle,
                    selectedJournalId === journal.id && journalItemActiveStyle
                  ]}
                  onPress={() => {
                    setSelectedJournalId(journal.id);
                    setUseJournal(true);
                ...
                >
                  <Text style={[
                    journalItemTextStyle,
                    selectedJournalId === journal.id && journalItemTextActiveStyle
                  ]}>
                    {journal.name}
                  </Text>
                </TouchableOpacity>
